...
export default function ClocheMessage({ showDropdown, onToggle }) {
    ...
    const getMessagesRoute = () => {
        if (isAdmin) {
            return '/admin/messages-gestionaires';
        } else if (isGestionaire) {
            return '/gestionaire/messages';
        } else {
            return '/formateur/messages';
        }
    };
    ...
    const fetchMessages = useCallback(async (showLoading = false) => {
        ...
        try {
            // Use different API endpoint for admin (only gestionaires messages)
            const apiEndpoint = isAdmin ? '/api/admin/messages-gestionaires' : '/api/messages';
                        ...
                        {loading ? (
                            ...
                            <div className="p-1.5">
                                {messages.slice(0, 5).map((message) => (
                                    <Link
                                        key={message.id}
                                        to={`${getMessagesRoute()}?user=${message.expediteur_id}`}
