...
export default function ClocheNotificationFormateur({ showDropdown, onToggle }) {
    ...
    const getNotificationLink = (notification) => {
        switch (notification.type) {
            // Application notifications - All redirect to demandes de formation page
            case 'application_created':
            case 'application_accepte':
            case 'application_accepted':
            case 'application_rejected':
            ...
            // Training notifications - Redirect to regular formations page
            case 'training_created':
            case 'training_updated':
                return '/formateur/stages/regulier';
            
            // Pedagogical training notifications - Redirect to pedagogical training page
            case 'pedagogical_training_started':
            case 'pedagogical_training_completed':
            ...
            // Promotion training notifications - Redirect to PSP1 or PSP2 based on message
            case 'promotion_training_started':
            case 'promotion_training_due':
            case 'promotion_training_cancelled':
            case 'promotion_dossier_submitted':
            case 'promotion_dossier_accept':
            case 'promotion_dossier_accepte':
            case 'promotion_dossier_reject_resubmit':
            ...
            case 'promotion_dossier_refuse':
            case 'promotion_dossier_resubmitted':
                // Determine if PSP1 or PSP2 from notification message or type
                if (notification.message && (notification.message.includes('10 ans') || notification.message.includes('10_ans') || notification.message.includes('10 ans'))) {
                    return '/formateur/stages/psp2';
                }
                // Default to PSP1 for 5 ans or if cannot determine
                return '/formateur/stages/psp1';
            
            // Diploma notifications - Redirect to diplomas page
            case 'diploma_issued':
    ...
    return (
        <div className="relative">
            <button
                ...
                className="relative p-1.5 sm:p-2 text-white/80 hover:text-white transition-colors"
            >
            ...
            {showDropdown && buttonRef && createPortal(
                <div 
                    ...
                    className="fixed w-[min(20rem,calc(100vw-1rem))] max-w-[calc(100vw-1rem)] bg-white/95 dark:bg-gray-900/95 backdrop-blur-xl rounded-2xl shadow-2xl border border-gray-200/50 dark:border-gray-700/50 z-[99999] overflow-hidden"
                    style={dropdownPos}
                >
                    {/* En-tête Moderne */}
                    ...
                    <div className="max-h-[min(24rem,calc(100dvh-8rem))] overflow-y-auto overflow-x-hidden custom-scrollbar">
                        {loading ? (
