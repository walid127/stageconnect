...
export default function PSP2() {
    ...
    const fetchPromotionTraining = async () => {
        ...
        try {
            ...
            if (response.data && response.data.data) {
                ...
                // Afficher seulement si la formation de promotion 10 ans existe
                if (data.has_promotion_training_10 && data.formation_promotion_10) {
                    setPromotionTraining({
                        ...data.formation_promotion_10,
                        has_schedule_file: data.has_schedule_file || false,
                    });
                } else {
    ...
    return (
        <div className="app-container">
                ...
                <div className="hero-content">
                    <div className="text-center">
                        <h1 className="hero-title">
                            PSP2 - Formation de Promotion (10 ans)
                        </h1>
                ...
                <div className="bg-white/95 dark:bg-[#161615]/95 rounded-xl shadow-md sm:rounded-2xl sm:shadow-lg border border-white/20 dark:border-[#3E3E3A]/50 overflow-hidden">
                    {loadingPromotion ? (
                        ...
                        <div className="p-8 text-center">
                            ...
                            <h3 className="text-xl font-semibold text-[#1b1b18] dark:text-[#EDEDEC] mb-2">Aucune formation de promotion 10 ans disponible</h3>
                            <p className="text-[#78786c] dark:text-[#9D9D99]">Aucune formation de promotion 10 ans (PSP2) disponible pour le moment.</p>
                        </div>
                    ) : (
                        <div className="p-6">
                            <div className="bg-white/95 dark:bg-[#161615]/95 rounded-xl shadow-md sm:rounded-2xl sm:shadow-lg border border-white/20 dark:border-[#3E3E3A]/50 overflow-x-auto">
                                {/* Table Header */}
                                <div className="min-w-[900px] sc-table-header-gradient px-3 py-2.5 sm:px-6 sm:py-3">
                                            ...
                                            <div className="col-span-3 text-center">
                                                {promotionTraining ? (
                                                    <div className="text-center space-y-1">
                                                        {promotionTraining.date_deb && (
                                                            <p className="text-xs text-[#78786c] dark:text-[#9D9D99]">
                                                                <span className="font-medium">Début:</span> {new Date(promotionTraining.date_deb).toLocaleDateString('fr-FR')}
                                                            </p>
                                                        )}
                                                        {promotionTraining.date_fin && (
                                                            <p className="text-xs text-[#78786c] dark:text-[#9D9D99]">
                                                                <span className="font-medium">Fin:</span> {new Date(promotionTraining.date_fin).toLocaleDateString('fr-FR')}
                                                            </p>
                                                        )}
                                                    </div>
                                                ) : (
                                            ...
                                            <div className="col-span-2 text-center">
                                                {promotionTraining ? (
                                                    <span className={`inline-flex px-3 py-1 text-xs rounded-full font-medium ${
                                                        promotionTraining.statut === 'en_cours' 
                                                            ? 'bg-blue-100 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400'
                                                            : promotionTraining.statut === 'termine'
                                                            ? 'bg-green-100 dark:bg-green-900/20 text-green-700 dark:text-green-400'
                                                            : 'bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-400'
                                                    }`}>
                                                        {promotionTraining.statut === 'en_cours' ? 'En cours' : promotionTraining.statut === 'termine' ? 'Terminé' : 'En attente'}
                                                    </span>
                                                ) : (
                                                    <span className="text-xs text-gray-400 dark:text-gray-500">-</span>
                                                )}
            ...
            {/* Details Modal */}
            {showDetailsModal && promotionTraining && (
                    ...
                    <div className="bg-white dark:bg-[#161615] rounded-2xl shadow-2xl border border-white/20 dark:border-[#3E3E3A]/50 w-full max-w-2xl max-h-[90vh] overflow-y-auto my-4">
                        <div className="bg-gradient-to-r from-purple-500 to-indigo-500 p-6 text-white rounded-t-2xl">
                            <div className="flex justify-between items-center">
                                <div>
                                    <h2 className="text-xl font-bold">Détails de la Formation de Promotion PSP2</h2>
                                    <p className="text-white/80 text-sm">{promotionTraining.titre}</p>
                                </div>
                                <button
                        ...
                        <div className="p-6 space-y-4">
                            <div>
                                ...
                                <p className="text-sm font-semibold text-[#1b1b18] dark:text-[#EDEDEC]">
                                    {promotionTraining.titre}
                                </p>
                            </div>
                            {promotionTraining.description && (
                                <div>
                                    <label className="block text-sm font-medium text-[#78786c] dark:text-[#9D9D99] mb-2">
                                        Description
                                    </label>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <p className="text-sm text-[#1b1b18] dark:text-[#EDEDEC]">
                                        {promotionTraining.date_deb ? new Date(promotionTraining.date_deb).toLocaleDateString('fr-FR') : '-'}
                                    </p>
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-[#78786c] dark:text-[#9D9D99] mb-2">
                                    ...
                                    <p className="text-sm text-[#1b1b18] dark:text-[#EDEDEC]">
                                        {promotionTraining.date_fin ? new Date(promotionTraining.date_fin).toLocaleDateString('fr-FR') : '-'}
                                    </p>
                                </div>
                            </div>
                            {promotionTraining.lieu && (
                                <div>
                                    <label className="block text-sm font-medium text-[#78786c] dark:text-[#9D9D99] mb-2">
                                        Lieu
                                    </label>
